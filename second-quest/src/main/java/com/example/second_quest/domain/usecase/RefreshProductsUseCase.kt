...
class RefreshProductsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(needToRefreshDB: Boolean = false): Result<List<Product>> {
        if (!connectivityChecker.isNetworkAvailable()) {
            return Result.Error(Throwable("No internet connection"))
        }
        return runSuspendCatching { productRepository.refreshProducts(needToRefreshDB) }.fold(onSuccess = {
                Result.Success(it)
            },
            onFailure = {
                Result.Error(it)
